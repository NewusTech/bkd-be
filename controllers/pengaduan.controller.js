...
module.exports = {
  createPengaduan: async (req, res) => {
    try {
      ...
      res
        .status(201)
        .json(response(201, "success create pengaduan", pengaduanCreate));
    } catch (err) {
  ...
  getPengaduanById: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json(response(200, "success get pengaduan by id", pengaduanGet));
    } catch (err) {
      res.status(500).json(response(500, "internal server error", err));
      console.log(err);
    }
  ...
  updatePengaduan: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json(response(200, "success update pengaduan", pengaduanAfterUpdate));
    } catch (err) {
      res.status(500).json(response(500, "internal server error", err));
      console.log(err);
    }
  ...
  pdfPengaduan: async (req, res) => {
    try {
      ...
      const bidangInfo = bidangGet?.nama
        ? `<p>Bidang : ${bidangGet?.nama}</p>`
        : "";
  ...
  getReportPermasalahan: async (req, res) => {
    try {
      ...
      if (
        req.user.role === "Admin Instansi" ||
        req.user.role === "Admin Verifikasi" ||
        req.user.role === "Admin Layanan"
      ) {
      ...
      const pagination = generatePagination(
        ...
        page,
        limit,
        "/api/user/bidang/report/pengaduan"
      );
