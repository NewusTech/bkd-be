...
module.exports = {
  ...
  createGaleri: async (req, res) => {
    try {
      ...
      res
        .status(201)
        .json(response(201, "success create galeri", GaleriCreate));
    } catch (err) {
    ...
    try {
      ...
      if (search) {
        whereCondition[Op.or] = [
          { title: { [Op.like]: `%${search}%` } }, // Menggunakan 'LIKE' untuk MySQL, gunakan 'like' untuk PostgreSQL
        ];
      }
  ...
  getGaleriBySlug: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json(response(200, "success get Galeri by slug", GaleriGet));
    } catch (err) {
      res.status(500).json(response(500, "internal server error", err));
      console.log(err);
    }
  ...
  updateGaleri: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json(response(200, "success update galeri", GaleriAfterUpdate));
    } catch (err) {
      res.status(500).json(response(500, "internal server error", err));
      console.log(err);
    }
