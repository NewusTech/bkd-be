...
const { User, User_spouse, Role, Bidang, sequelize } = require('../models');
...
module.exports = {
    ...
    getUserDataSpouse: async (req, res) => {
        try {
    ...
    createUserSpouse: async (req, res) => {
        ...
        try {
            ...
            const schema = {
                ...
                tanggal_pernikahan: { type: "string", optional: true },
            ...
            // Proses setiap item dalam array user data spouse
            for (let userSpouse of userSpouses) {
                // Masukkan user_id dari req.user ke dalam setiap objek spouse
                ...
                let userspousesCreate = await User_spouse.create(userSpouse);
                createdSpouses.push(userspousesCreate);
            }
            ...
            res.status(200).json({
                status: 200,
                message: 'User data pasangan created successfully',
                data: createdSpouses
            });
    ...
    deleteUserSpouse: async (req, res) => {
        const transaction = await sequelize.transaction();
