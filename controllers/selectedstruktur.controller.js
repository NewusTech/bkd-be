...
module.exports = {
    ...
    createSelectedStruktur: async (req, res) => {
        try {
            ...
            // Buat select blog
            let selectedstrukturCreate = await Selected_struktur.create(selectedstrukturCreateObj);
      
            res.status(201).json(response(201, 'success create select selected', selectedstrukturCreate));
          } catch (err) {
                ...
                include: [
                    {
                        model: Bkd_struktur,
                        as: 'struktur',
                        attributes: ['id', 'nama', 'slug', 'jabatan', 'image', 'createdAt', 'updatedAt'],
                        where: {
    ...
    updateSelectedStruktur: async (req, res) => {
        try {
            ...
            if (!selectedStruktur) {
                return res.status(404).json(response(404, 'Selected struktur not found'));
            }
            ...
            res.status(200).json(response(200, 'success update selected struktur', updatedStruktur));
    ...
    deleteSelectedStruktur: async (req, res) => {
        try {
            ...
            let selectedstrukturGet = await Selected_struktur.findOne({
                where: {
                    id: req.params.id,
                    deletedAt: null
                }
            ...
            await Selected_struktur.update({ deletedAt: new Date() }, {
                where: {
                    id: req.params.id
                }
            });
