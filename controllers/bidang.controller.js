...
module.exports = {
  ...
  createBidang: async (req, res) => {
    try {
      ...
      res
        .status(201)
        .json(response(201, "success create bidang", bidangCreate));
    } catch (err) {
    ...
    try {
      ...
      if (search) {
        whereCondition[Op.or] = [
          { nama: { [Op.like]: `%${search}%` } }, // Ganti like dengan LIKE untuk MySQL
          {
  ...
  getBidangBySlug: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json(response(200, "success get bidang by slug", formattedBidangGets));
    } catch (err) {
      res.status(500).json(response(500, "internal server error", err));
      console.log(err);
    }
  ...
  updateBidang: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json(response(200, "success update bidang", bidangAfterUpdate));
    } catch (err) {
      res.status(500).json(response(500, "internal server error", err));
      console.log(err);
    }
